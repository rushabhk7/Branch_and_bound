	...
	if(pid == 0){
		...
		while(num_idle < num - 1){
			...
			if(!strcmp(msg,"GET_SLAVES")){
				...
				T2 cand_sol = T1::decode_sol(buffer,pos);
				if( ( p.get_goal() && cand_sol.get_cost() > best.get_cost()) ||
						(!p.get_goal() && cand_sol.get_cost() < best.get_cost()) ){
					best = cand_sol;
					best_cost = cand_sol.get_cost();
				}
			}
